...
function Day9(INPUT, keyinput)
  ...
  while (status == "suspended" or status == "") do
    if (status == "") then
      retval, out = coroutine.resume(intcode, INPUT, keyinput)
    else
      retval, out = coroutine.resume(intcode, {})
    end
    ...
    if (retval == false) then -- pcall error case...
      ...
      if (status ~= "dead") then
        ...
        table.insert(out_cat, out)
      else
        return out, table.concat(out_cat, ",")
      end
    end
  end
end
...
assert(select(2, Day9{109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99}) == "109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99")
assert(Day9({1102,34915192,34915192,7,4,7,99,0}) == 1219070632396864)
assert(Day9({104,1125899906842624,99}) == 1125899906842624)
assert(Day9(INPUT, {1}) == 3507134798)
