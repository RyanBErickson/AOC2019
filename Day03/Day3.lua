...
-- Read Test A Inputs
TESTA = {"R8,U5,L5,D3", "U7,R6,D4,L4"}
TESTA = { "R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83" }
TESTA = { "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7" }
...
function Day3A(input)
  ...
  local markmap = function(dx, dy, mark)
	  x, y = x+dx, y+dy
	  ...
	  if (not map[y][x]) then
		  map[y][x] = mark
	  elseif (map[y][x] ~= mark) and (map[y][x] ~= 'o') then
		  map[y][x] = '+'
		 local distance = math.abs(y - 1) + math.abs(x - 1)
  ...
  local xmap = {U = {0, -1}, D = {0, 1}, L = {-1, 0}, R = {1, 0}}
  markmap(0, 0, 'o')

  for i, path in pairs(input) do
	  x, y = 1, 1
	  path = path .. ","
	  for walk in path:gfind("(.-),") do
		  local _, _, dir, len = walk:find("(.)(%d+)")
		  for _ = 1, len do
			  markmap(xmap[dir][1], xmap[dir][2], tostring(i))
		  end
	  end
  end
  ...
  -- print map
  if (false) then
	  for _y = miny - 1, maxy + 1 do
		  local out = {}
		  for _x =  minx - 1, maxx + 1 do
			  table.insert(out, (map[_y] or {})[_x] or '.')
		  end
		  print(table.concat(out))
	  end
  end

  dbg("min distance: " .. mindist)
  return mindist
end
...
function Day3B(input)
  ...
  for i, path in pairs(input) do
    ...
    steps = steps + 1
	      ...
	      map[y] = map[y] or {}
	      if (not map[y][x]) then
		      map[y][x] = {s = mark, steps = steps}
	      elseif (map[y][x].s ~= mark) and (map[y][x].s ~= 'o') then
		      local cursteps = map[y][x].steps
		      map[y][x] = {s = '+', steps = cursteps + steps}
	      end
      --print("X: " .. x .. " Y: " .. y .. " val: " .. map[y][x])
	end
      elseif (dir == 'D') then
        for dy = 1, len do
	      y = y + 1
    ...
    --dbg("minx: " .. minx .. " maxx: " .. maxx .. " miny: " .. miny .. " maxy: " .. maxy)
  end
    local mindist = math.huge
    for _y = miny - 1, maxy + 1 do
	    local out = {}
	    for _x =  minx - 1, maxx + 1 do
	      local point = ((map[_y] or {})[_x] or { s = '.' })
              if (point.s == '+') then
		 local steps = point.steps or math.huge
		 if (steps < mindist) then
		    mindist = steps
		 end
		 --dbg("x: " .. _x .. " y: " .. _y .. " distance: " .. distance)
	      end
	      if ((point.steps or 0) == 0) then
	      table.insert(out, point.s .. "[   ] ")
      else
	      table.insert(out, point.s .. "[" .. string.format("%03d", point.steps or 0) .. "] ")
      end
	    end
	    --print(table.concat(out))
    end
